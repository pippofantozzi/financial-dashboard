    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          ...
          onClick={() => setOpen(!open)} // Explicitly toggle on click
        >
          ...
          <CommandList className="max-h-[300px] overflow-y-auto">
            ...
            <CommandGroup>
              {categories.map((category) => (
                <CommandItem
                  key={category.id}
                  value={category.name} // Use name for filtering
                  onSelect={() => { // Simplified selection handler
                    onChange(category.id); // Always set the ID directly
                    setOpen(false);
                  }}
                    ...
                    {showNewCategoryInput ? (
                        <div className="flex flex-col space-y-1.5 px-1 pb-1">
                         ...
                         <CommandItem
                              ...
                              className="text-muted-foreground hover:text-foreground cursor-pointer hover:bg-accent hover:text-accent-foreground"
                           >
